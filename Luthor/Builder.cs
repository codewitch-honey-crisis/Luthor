...
internal sealed class Builder
{
    ...
    // Returns codepoint-level states; state 0 is the start state.
    // Rule 0 has the highest priority when several rules accept the same length.
    internal static List<DfaState> Build(IEnumerable<string> rules)
    {
        ...
        foreach(string rule in rules)
        {
            ...
            try
            {
                f = builder.ParseRule(rule);
            }
            catch(FormatException fe)
            {
                throw new FormatException($"{fe.Message} (rule #{r + 1})");
            }
